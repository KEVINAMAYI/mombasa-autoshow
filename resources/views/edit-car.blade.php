...
@include('layouts.partials.navbar',['pagetitle' => 'Update a car/PSV'])
...
<div id="container">
	<div id="page-contents">
    	...
    	<div id="car-form">
        	<form id="create-car-form"  action="update-vehicle/{{ $car->id }}" method="POST" class="row g-3" enctype="multipart/form-data">
            @csrf
              <div class="col-md-4">
                <label for="carfor" class="form-label"><strong>Submit car for?</strong></label>
                <select  id="carfor" name="carfor" class="form-select">
                    <option value="{{ $car->carfor }}" selected>{{ $car->carfor }}</option>
                    <option value="Car of the year award">Car of the year award</option>
                    <option value="PSV of the year award">PSV of the year award</option>
                    <option value="Car for Auction">Sell a Car(Public Auction)</option>
                </select>
              </div>
              <div class="col-md-4">
              ...
              <div class="col-md-4">
                ...
                <select id="make" name="make" class="form-select" required>
                  <option value="{{ $car->make}}" selected>{{ $car->make }}</option>
              ...
              <div class="col-md-4">
                ...
                <input type="number" min="1900" max="2023" step="1" name="manufacture_year" id="manufacture_year" value="{{ $car->manufacture_year }}" class="form-control" id="engine_cc"  required name="engine_cc" placeholder="1500">
              </div>
              <div class="col-md-4">
                <label for="location" class="form-label">Location</label>
                <select id="location" name="location" class="form-select" required>
                  <option value="{{ $car->location }}" selected>{{ $car->location }}</option>
              ...
              <div class="col-md-4">
                <label for="engine_cc" class="form-label">Engine cc</label>
  				<input type="text" class="form-control" id="engine_cc" value="{{ $car->engine_cc }}" required name="engine_cc" placeholder="1500">
              </div>
              <div class="col-md-4">
              ...
              <div class="col-md-4">
                ...
                <select id="fuel_type" name="fuel_type" class="form-select" required>
                  <option value="{{ $car->fuel_type }}" selected>{{ $car->fuel_type }}</option>
              ...
              <div class="col-md-4">
                <label for="vehicl_reg" class="form-label">Vehicle Registration</label>
  				<input type="text" class="form-control" id="vehicle_reg" value="{{ $car->vehicle_reg ?? "" }}" required name="vehicle_reg" placeholder="KAA 000A">
              </div>
              <div class="col-md-4">
                <label for="price" class="price form-label">Price(Kshs)</label>
  				<input type="number" class="form-control" id="price" value="{{ $car->price ?? "" }}" name="price" placeholder="800,000 (Car for Auction)">
              </div>
              <div class="col-md-4">
                <label for="vehicle-name" class="vehicle_name form-label">Vehicle Name</label>
  				<input type="text" class="form-control" id="vehicle_name" value="{{ $car->vehicle_name ?? "" }}" name="vehicle_name" placeholder="Vehicle Name (PSV)">
              </div>
              <div class="col-md-4">
                <label for="sacco-name" class="sacco_name form-label">Sacco Name</label>
  				<input type="text" class="form-control" id="sacco_name" value="{{ $car->sacco_name ?? "" }}" name="sacco_name" placeholder="Sacco Name (PSV)">
              </div>
              <div class="col-md-4">
                <label for="route" class="route form-label">Route</label>
  				<input type="text" class="form-control" id="route"  value="{{ $car->route ?? "" }}" name="route"  placeholder="Route (PSV)">
              </div>
              <div class="col-12">
                <label for="description" class="form-label">Description(What is unique about this car?)</label>
  				<textarea class="form-control" id="description" required name="description" rows="3">{{ $car->description ?? ""}}
          </textarea>
              </div>
              <div class="col-md-4">
              ...
              <div class="col-12">
                <button id="createcar"  type="submit" class="btn btn-primary">Update</a>
              </div>
            </form>
